...
def normalized_del_residual_loss(
    ...
    # Average squared whitened residual coordinates per triplet, then normalize
    # each sequence against the detached baseline sequence energy.
...
def build_route_mean(
    ...
    cached_c0: Optional[torch.Tensor],
    recenter_source: str,
    recenter_eps: float,
    f1_delta_mode: str,
    kp_min: float,
    ...
    mu_nom_frozen = None
    c0 = None
    if cached_frozen_nominal is not None and cached_c0 is not None:
    ...
    if route == 'f1':
        if mu_nom_frozen is None or c0 is None:
        ...
        pred_mean = (c0 + delta).clamp(min=kp_min, max=kp_max)
    elif route == 'c2':
    ...
    return pred_mean, {
        'c0': c0.detach() if c0 is not None else None,
...
def one_pass(
    ...
    if frozen_anchor_cache is not None and sample_indices is not None:
        ...
        cached_c0 = frozen_anchor_cache['c0'].index_select(0, sample_indices).to(
            device,
            non_blocking=True,
        )
    ...
    anchor_mean = route_components['c0']
    if anchor_mean is not None:
        anchor_mean = anchor_mean.clamp(min=kp_min, max=kp_max)
    ...
    if reorder_method != 'none':
        ...
        if anchor_mean is not None:
            anchor_perms = compute_temporal_permutations(anchor_mean.detach(), reorder_method)
            anchor_mean = apply_temporal_permutation(anchor_mean, anchor_perms)
    ...
    if anchor_mean is not None:
        anchor_assign = sequence_level_hungarian_assign(anchor_mean.detach(), gt_pos.detach(), gt_mask=in_cam)
        anchor_mean = gather_slots(anchor_mean, anchor_assign)
    ...
    q_anchor_world = None
    if anchor_mean is not None:
        q_anchor_world = matched_latent_yx_to_d6_true_state(
            anchor_mean.detach(),
            args,
            gt_similarity_transform=gt_similarity_transform,
        )
    ...
    if args.objective == 'c1_dyn' and compute_dynamics_loss:
        ...
        if q_anchor_world is None:
            raise RuntimeError(
                "objective='c1_dyn' requires a frozen c0 baseline for DEL normalization, "
    ...
    with torch.no_grad():
        ...
        if q_anchor_world is not None:
            q_anchor_world = q_anchor_world.detach()
    ...
    residual_t_c0 = diagnostic_weak_del_residual_loss(
        lagrangian_model=lagrangian_model,
        q_seq=q_anchor_world,
        weak_window=args.dyn_weak_window,
    )
    ...
    residual_delta_q_theta_minus_c0 = None
    if residual_t_q_theta is not None and residual_t_c0 is not None:
        residual_delta_q_theta_minus_c0 = residual_t_q_theta - residual_t_c0
    ...
    out = {
        ...
        'diag_res_t_c0': _metric_or_zero(residual_t_c0),
        ...
        'diag_res_delta_q_theta_minus_c0': _metric_or_zero(residual_delta_q_theta_minus_c0),
    }
...
def run_epoch(
    ...
    agg = {
        ...
        'diag_res_delta_q_theta_minus_c0': 0.0,
    }
    n = 0
